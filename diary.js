...
(window.requestIdleCallback || setTimeout)(async function load() 
{
    ...
    function doStuff(node) {
        ...
        if (grammar.ig.test(textContent)) textContent = textContent.replace(grammar.ig, ig)
        if (grammar.istg.test(textContent)) textContent = textContent.replace(grammar.istg, istg)
        if (grammar.idk.test(textContent)) textContent = textContent.replace(grammar.idk, idk)
        if (grammar.ik.test(textContent)) textContent = textContent.replace(grammar.ik, ik)
        if (grammar.idc.test(textContent)) textContent = textContent.replace(grammar.idc, idc)
        if (grammar.bc.test(textContent)) textContent = textContent.replace(grammar.bc, because)
        if (grammar.im.test(textContent)) textContent = textContent.replace(grammar.im, "I'm")
        if (grammar.i.test(textContent)) textContent = textContent.replace(grammar.i, 'I')
        if (textContent !== node.textContent) node.textContent = textContent
    }
    function idc(str) {
        let { 0: i, 1: d, 2: c } = str
