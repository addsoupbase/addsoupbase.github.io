...
// console.time('css.js');
//// self.css = 
(function CSSSetup(inModule, w, sym, S, defer, D, O) {
    ////'use strict'
    var id = '/*stylesheet auto-generated by css.js*/'
    if (sym in w || w[id] instanceof HTMLStyleElement) return w[sym]
    addEventListener('error', function n(e) { var a = e.filename; if (a.endsWith('.js')) { [].forEach.call(D.querySelectorAll('script[nomodule]'), function (s) { D.head.appendChild(D.createElement('script')).src = s.src }); removeEventListener('error', n) } })
    ...
    return css
}(!this, self, Symbol.for('[[CSSModule]]'), sessionStorage, (self.requestIdleCallback && function (c) { return requestIdleCallback(c, { timeout: 2000 }) }) || (self.scheduler && scheduler.postTask && function (c) { return scheduler.postTask(c, { priority: 'background' }) }) || self.queueMicrotask || self.setImmediate || setTimeout, document, Object))
