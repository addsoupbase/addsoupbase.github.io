...
(function (w, sym) {
    return cssSetup(sessionStorage, !this, (window.requestIdleCallback && function (callback) { return requestIdleCallback(callback, { timeout: 2000 }) }) || (window.scheduler && scheduler.postTask && function (callback) { return scheduler.postTask(callback, { priority: 'background' }) }) || window.queueMicrotask || window.setImmediate || setTimeout)
    function cssSetup(S, inModule, late) {
        ...
        if (canWrite && top === window) {
            ...
            document.write('<p style="transform:scale(0);display:table-cell;z-index:-9999;" id="' + date + '" aria-hidden="true">.</p>')
        ...
        /*function fromCSS(str) {
            var a = str.trim().split(';').filter(Boolean)
                , o = {}
