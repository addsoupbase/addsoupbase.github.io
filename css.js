...
(function CSSSetup(inModule, w, sym, S, defer, D) {
    ...
    var id = '/*stylesheet auto-generated by css.js*/'
    if (sym in w || w[id]instanceof HTMLStyleElement) return w[sym]
    var d = Object.defineProperty//, script = document.currentScript; 
    ''.startsWith || d(String.prototype, 'startsWith', { value: function (s, p) { return this.slice(p = p | 0, (s += '').length + p) === s } });[].find || d(Array.prototype, 'find', { value: function (c, t, m, z) { for (var i = 0, l = (m = this).length; i < l; ++i) { z = m[i]; if (c.call(t, z)) return z } } }); ''.includes || (String.prototype.includes = function (a, b) { return !!~this.indexOf(a, b) })
    ...
    , toValue = function (parse) {
        if (!parse) {
            ...
            typeof Symbol === 'function' && props.push(Symbol.iterator)
            var proto = { //valueOf: { value: function () { return parseFloat(this.toString()) } } 
            }
            var hi = function (prop) {
                proto[prop] = {
                    value: function () {
                        return ''[prop].apply(this.toString(), arguments)
                    }
                }
            }
        ...
        return a
        function a(p, v) {
           return Object.defineProperty(Object.defineProperties(parse(p, v.replace(imp, '')), proto), 'length', { value: v.length })
        }
        function b(p, v) { return v.replace(imp, '') }
    }(self.CSSStyleValue && CSSStyleValue.parse)
    ...
    function CSSObj(input) {
        ...
        for (var p in input) {
            ...
            if (p.startsWith('--')) Object.defineProperty(out, '__' + toCaps(p.substring(2)), { value: v })
                ...
                var a = toCaps(p)
                hasOwn(a) || Object.defineProperty(out, a, { value: v })
            }
        }
        return out
    }
        ...
        function load(text) {
            ...
            while (match = parse.exec(text)) {
                ...
                if (rule) {
                    ...
                    var lines = og.split('\n')
                    var line = href + ':' + (lines.findIndex(function (o) { return o.includes(selector) }) + 1)
                    if (!Object.keys(c).length) console.warn('Empty ruleset:', line)
