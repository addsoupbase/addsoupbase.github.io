const sym = Symbol.for("🔔")
    ...
    function verifyEventName(target, name) {
        ...
        if (/^domcontentloaded$/i.test(name) &&
    ...
    export function on(target, events, useHandler) {
        ...
        try {
            ...
            for (let eventName in events) {
                ...
                let signal = eventName.match(/\d+/)?.[0]
                ...
                eventName = verifyEventName(target, eventName.replace(/[_$^%&!?\d]|bound /g, ''))
                ...
                function ProxyFunction(...args) {
                    let { 0: event } = args
                    controller && args.push(abc, def)
                    if (onlyTrusted && event.isTrusted || !onlyTrusted)
