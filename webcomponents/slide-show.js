    ...
    class SlideShow extends HTMLElement {
        ...
        static preload(...sources) {
            ...
            for (let { framesX = 1, framesY = 1, src, duras, image, reversed } of sources) {
                ...
                n.decoding = 'sync'
                n.src = src
                function load(o, width, height, isSpecial) {
                    let vals = []
                    ...
                    bitmaps.set(s, out)
                    sheet.insertRule(`:host([src="${isSpecial ? src : s}"]){width:${width}px;height:${height}px}`, 1)
                    isSafari && sheet.insertRule(`:host([src="${isSpecial ? src : s}"]) div{width:${width}px;height:${height}px}`, 1)
                    return out
                }
...
ffmpeg -f concat -safe 0 -i list.txt \\
    ...
    sheet.replaceSync(`#sprite{display:flex}svg,div{width:100%}div{outline:1px solid transparent;contain:paint;pointer-events:all;overflow:clip;transform:translate(-50%,-50%);}foreignObject{y:calc((rem(calc(var(--index,0)*var(--frame-h,0)),var(--height,0))*-1px))}:host{isolation:isolate;user-select:none;-webkit-user-select:none;-moz-user-select:none;touch-action:pinch-zoom;pointer-events:none !important;transform-origin:0 0;display:flex;width:0;height:0;image-rendering:-moz-crisp-edges;image-rendering:-webkit-optimize-contrast;image-rendering:pixelated}:host(:--broken){width:32px;height:32px;content:attr(aria-label);background-size:cover;}:host(:state(--broken)){width:32px;height:32px;content:attr(aria-label);background-size:cover;}:host(:--broken) div{${broken}}:host(:state(--broken)) div{${broken}}`)
