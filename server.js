...
function a() {
    ...
    addEventListener('beforescriptexecute', function (e) {
        if (e.target === n) return
        performance.mark(e.target.src + ':start')
    })
    addEventListener('afterscriptexecute', function (e) {
        if (e.target === n) return
        let { target } = e
    ...
    addEventListener('load', function () {
        ...
        var time = performance.measure('page load time', 'start', 'end')
        console.debug('Page loaded in ' + (time.duration | 0) + 'ms')
    }, { once: true })
}
