...
(function CSSSetup(inModule, sym, defer, D, O, id, y, ads) {
    ...
    var u = Element.prototype, $ = O.defineProperty
    'currentCSSZoom' in u || $(u, 'currentCSSZoom', { value: 1 })
    'currentScript' in HTMLDocument.prototype || $(HTMLDocument.prototype, 'currentScript', { get: function () { var z = this.getElementsByTagName('script'); return this.readyState === 'loading' ? z[z.length - 1] || null : null } })
    u.matches || $(u, 'matches', { value: u.webkitMatchesSelector || u.msMatchesSelector || u.mozMatchesSelector || u.oMatchesSelector })
    try { var S = sessionStorage } // it just throws on read if storage is denied
    catch (e) { //$devconsole.warn(e) 
        var e = function () { },
            x = e.prototype
        x.constructor = Storage
        delete window.sessionStorage
        delete window.localStorage
        var QUE = false
        var QQ = function () {
            let a = JSON.stringify(sessionStorage)
            SET(a)
        }
        var SET = function (v) {
            try {
        ...
        var QU = function () {
            if (!QUE) {
                QUE = true
                Promise.resolve().then(QQ)
            }
        }
        $(x, Symbol.toStringTag || 'toStringTag', {
            value: 'FakeStorage'
        });
        $(x, 'length', {
            get: function () {
                return O.keys(this).length
            }
        });
        $(x, 'clear', {
            value: function () {
        ...
        $(x, 'getItem', {
            value: function (k) {
                var n = O.getOwnPropertyDescriptor(this, k);
                QU()
        ...
        if (typeof Proxy == 'function') var y = e,
            e = function (z) {
                var a = new y
                if (z) try { Object.assign(a, JSON.parse(top.name)) } catch (e) { }
                return new Proxy(a, {
                    set: function (t, p, v) {
                        t.setItem(p, v);
                        return 1
                    ...
                    get: function (t, p) {
                        return p in y.prototype ? y.prototype[p] : t.getItem(p)
                    }
                })
            };
        window.sessionStorage = new e(true)
        window.localStorage = new e
    }
    if (!inModule) {
        var onerror = function (e) { if (e.filename.endsWith('.js')) { D.write = function (l) { return function () { var z = D.currentScript; z ? z.src ? z.insertAdjacentHTML('afterend', [].join.call(arguments, '')) : l.apply(D, arguments) : D.firstElementChild.insertAdjacentHTML('beforeend', [].join.call(arguments, '')) } }(D.write);[].forEach.call(D.querySelectorAll('script[nomodule]'), function (s) { var a = D.head.appendChild(D.createElement('script'));a.async = false;a.src = s.src }); removeEventListener('error', onerror) } }
    ...
    if (!newName) {
        ...
        o[W('img,image,input[type="image"]')] = {
            // 'align-content': 'end',
            'text-align': 'center',
            height:'auto',
        ...
        o[W('img,video,canvas,svg,picture,image,input[type="image"]')] = {
    ...
    y[sym] || $(y, sym, { value: css, enumerable: 1 })
    return css
